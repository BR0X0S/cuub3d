...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/24 17:37:06 by oumondad          #+#    #+#             */
/*   Updated: 2024/12/24 17:37:49 by oumondad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line_dda(t_game *cub, t_line *line, double angle, double length, int color)
{
	...
	x = cub->player.pixel_x + P_SIZE / 2;
	y = cub->player.pixel_y + P_SIZE / 2;
...
double	raycast_dda(t_game *cub, double angle, int *hit_x, int *hit_y)
{
	...
	while (1)
	{
		...
		{
			if (hit_x) *hit_x = x; // Set the hit coordinates if requested
			if (hit_y) *hit_y = y;
			break;
		}
...
void	draw_fov_dda(t_game *cub, int num_rays, double fov_angle, int color)
{
	...
	for (int i = 0; i < num_rays; i++)
	{
		...
		draw_line_dda(cub, NULL, ray_angle, length, color);
	}
}
